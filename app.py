...
def index():
    ...
    return render_template('index.html', mse_without_scaling=mse_without_scaling, mse_with_scaling=mse_with_scaling)
...
def plot_png():
    ...
    plt.plot([y_train_encoded.min(), y_train_encoded.max()], [y_train_encoded.min(), y_train_encoded.max()], 'k--', lw=2)
    plt.xlabel('Valores Reales')
    plt.ylabel('Predicciones') ### hoyyy
    plt.title('Con Escalado')
    plt.legend()

    plt.tight_layout()

    # Guardar la gráfica en formato PNG en memoria
    output = BytesIO()
